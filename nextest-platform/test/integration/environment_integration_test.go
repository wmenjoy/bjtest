...
// setupTestEnvironmentWithFile creates test environment with file-based DB for concurrency tests
func setupTestEnvironmentWithFile(t *testing.T) (*gorm.DB, *gin.Engine, string) {
	// Create temporary file database (better for concurrent access than :memory:)
	tmpFile := fmt.Sprintf("/tmp/test_concurrency_%d.db", time.Now().UnixNano())
	...
	// Run migrations including multi-tenancy tables
	err = db.AutoMigrate(
		&models.Tenant{},
		&models.Project{},
		&models.TenantMember{},
	...
	// Add middleware to inject tenant headers for testing
	...
	// Create router group for API versioning
	rg := router.Group("/api/v2")

	testHandler.RegisterRoutes(rg)
	workflowHandler.RegisterRoutes(rg)
	envHandler.RegisterRoutes(rg)

	return db, router, tmpFile
}
...
func TestEnvironmentActivation_Concurrency(t *testing.T) {
	// Setup with file-based database for better concurrency support
	db, router, tmpFile := setupTestEnvironmentWithFile(t)
	defer func() {
		cleanupTestEnvironment(db)
		os.Remove(tmpFile)          // Clean up temp file
		os.Remove(tmpFile + "-shm") // Clean up WAL shared memory
		os.Remove(tmpFile + "-wal") // Clean up WAL file
	}()
	...
	if isActive, ok := activeEnv["isActive"]; ok {
		assert.True(t, isActive.(bool))
	} else {
		t.Errorf("isActive field not found in response: %+v", activeEnv)
	}
}
...
func TestVariableInjection_TypePreservation(t *testing.T) {
	...
	// Inject variables
	result, err := injector.InjectVariables(context.Background(), "default", "default", config, nil)
...
func (a *WorkflowExecutorAdapter) Execute(workflowID string, workflowDef interface{}, params *testcase.ExecutionParams) (*testcase.WorkflowResult, error) {
	// Convert testcase.ExecutionParams to workflow.ExecutionParams
	var wfParams *workflow.ExecutionParams
	if params != nil {
		wfParams = &workflow.ExecutionParams{
			TenantID:  params.TenantID,
			ProjectID: params.ProjectID,
		}
	}
	result, err := a.impl.Execute(workflowID, workflowDef, wfParams)
	if err != nil {
		return nil, err
	}
...
// addTenantHeaders adds required tenant and project headers to the request
func addTenantHeaders(req *http.Request) {

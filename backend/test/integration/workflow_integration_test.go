...
// setupWorkflowTestEnvironment creates a complete test environment with all components for workflow tests
func setupWorkflowTestEnvironment(t *testing.T) (*gin.Engine, *gorm.DB, *websocket.Hub) {
	// Setup database
	db, err := gorm.Open(sqlite.Open(":memory:"), &gorm.Config{})
	require.NoError(t, err)

	// Migrate all models including multi-tenancy tables
	err = db.AutoMigrate(
		&models.Tenant{},
		&models.Project{},
		&models.TenantMember{},
	...
	// Create a default active environment for tests
	...
	// Create environment service and variable injector
...
func TestMode1_WorkflowReference(t *testing.T) {
	router, db, _ := setupWorkflowTestEnvironment(t)
...
func TestWorkflowAPI_CRUD(t *testing.T) {
	router, _, _ := setupWorkflowTestEnvironment(t)
...
func TestWorkflow_ErrorHandling(t *testing.T) {
		...
		"definition": map[string]interface{}{
			...
			"steps": map[string]interface{}{
				...
				"step2": map[string]interface{}{
					...
					"dependsOn": []string{"step1"},
					"config": map[string]interface{}{
						"cmd":  "/nonexistent/command/that/will/fail", // Command that always fails (non-existent)
						"args": []string{},
					},
				},
			},
	...
	// Check if runId exists before type assertion
...
func TestWorkflow_RealTimeUpdates(t *testing.T) {
	router, db, hub := setupWorkflowTestEnvironment(t)
...
// setupWorkflowTestEnvironmentWithFile creates test environment with file-based DB for concurrency tests
func setupWorkflowTestEnvironmentWithFile(t *testing.T) (*gin.Engine, *gorm.DB, *websocket.Hub, string) {
	// Create temporary file database (better for concurrent access than :memory:)
	tmpFile := fmt.Sprintf("/tmp/test_workflow_concurrency_%d.db", time.Now().UnixNano())
